...
class ResentListAdapter(
    private var context: Context,
    private var items: MutableList<DataItem?>,
    private val listener: (DataItem) -> Unit
) :
    RecyclerView.Adapter<ResentListAdapter.ViewHolder>(), Filterable {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val itemView = LayoutInflater.from(parent?.context)
            .inflate(R.layout.resent_list_item, parent, false)
    ...
    inner class ViewHolder(row: View) : RecyclerView.ViewHolder(row) {
        ...
        init {
            this.tvTitle = row.findViewById<TextView>(R.id.tvName)
